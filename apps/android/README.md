...
| debug (`~/.android/debug.keystore`) — 패키지 `…drop.mobile` | `6B:45:06:0A:EA:62:47:E6:88:82:3A:4E:14:FA:24:1E:93:96:96:82` | 등록됨 (`627053596385-bn6h…`) |
| debug/profile — 패키지 `…drop.mobile.debug` | 위와 같은 지문 | 등록됨 (`627053596385-qf52…`) |

디버그 클라이언트는 콘솔에서 사람이 만들어야 한다 (GCP에 OAuth 클라이언트 생성 API가 없다).

디버그·프로필 빌드는 `applicationIdSuffix = ".debug"` 로 **다른 패키지명으로 깔린다** (BRU-209).
Google 은 `(패키지명, SHA-1)` 로 매칭하므로 지문이 같아도 패키지가 다르면 별도 클라이언트가 필요하다 —
그래서 디버그 항목이 두 줄이다. `.debug` 쪽을 지우면 디버그 빌드 로그인이 `ApiException: 10` 으로 죽는다.
